...
function nodemon(settings) {
  ...
  console.log('lib/nodemon@nodemon: config.load...');

  config.load(settings, function (config) {
    console.log('lib/nodemon@nodemon: config.loaded', config);
    if (!config.options.dump && !config.options.script && config.options.execOptions.exec === 'node') {
      if (!config.required) {
        console.log(help('usage'));
        process.exit();
      }
      console.log('lib/nodemon@nodemon: return 1');
      return;
    }
    ...
    if (config.options.dump) {
      ...
      utils.log._log('log', '--------------');
      if (!config.required) {
        console.log('lib/nodemon@nodemon: return 2');
        process.exit();
      }
    } else {
      config.run = true;
      ...
      console.log('lib/nodemon@nodemon: monitor.run');
...
console.log('lib/nodemon: config.required: ' + config.required);
